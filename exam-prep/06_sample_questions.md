# Official Sample Questions (Q1-Q12)

> Source: CCA-F Exam Guide v1, pages 25-31.
> These 12 questions are drawn from the official practice test and represent the format,
> difficulty level, and scenario structure of the actual exam.

**Exam format reminder:** 60 questions, 120 minutes, multiple choice (A/B/C/D), passing score 720/1000.
Questions are scenario-based — read the full scenario before choosing an answer.

---

## Scenario 1: Customer Support Resolution Agent

### Q1 — Tool prerequisite enforcement
...
**Repo reference:** `modules/module-1/04_programmatic_guardrails.py`, `modules/module-1/05_sdk_hooks.py`

**Key distinction:** Prompt instructions (B) and few-shot examples (C) both let the model *decide* whether to comply. A programmatic prerequisite (A) makes non-compliance structurally impossible — the tool call is blocked at the framework layer.

---

### Q2 — Tool description quality as primary selection mechanism
...
**Repo reference:** `modules/module-2/01_tool_design.py`

**Key principle:** Tool descriptions → tool selection. When two tools are confused, the fix is descriptions first, few-shot examples second, architecture changes third.

---

### Q3 — Escalation calibration with explicit criteria
...
**Repo reference:** `modules/module-5/02_escalation_patterns.py`

**Key anti-pattern:** Never use self-reported LLM confidence (B) or sentiment (D) as escalation triggers. Use objective, policy-based criteria with named trigger labels and few-shot examples.

---

## Scenario 2: Code Generation with Claude Code

### Q4 — Project-scoped slash commands
...
**Repo reference:** `modules/module-3/02_custom_commands_skills.py`, `exercises/exercise-2-team-workflow/.claude/commands/`

**Key distinction:**
- `.claude/commands/` = project scope, in repo, shared with team
- `~/.claude/commands/` = user scope, personal, not shared

---

### Q5 — Plan mode for architectural changes
...
**Repo reference:** `modules/module-3/04_plan_vs_direct.py`

**Plan mode trigger:** Multi-file changes + architectural decisions + multiple valid approaches = always enter plan mode.

---

### Q6 — Path-specific rules for test files
...
**Repo reference:** `modules/module-3/03_path_specific_rules.py`, `exercises/exercise-2-team-workflow/.claude/rules/`

**Key insight:** When a convention applies to files scattered across directories (like `*.test.tsx` everywhere), only glob-pattern rules in `.claude/rules/` can target them correctly. CLAUDE.md hierarchy is directory-bound.

---

## Scenario 3: Multi-Agent Research System

### Q7 — Coordinator decomposition scope
...
**Repo reference:** `modules/module-1/02_multi_agent_orchestration.py`

**Diagnostic approach:** When all subagents succeed but the output is incomplete, look at the coordinator's decomposition first. Subagent success ≠ system success if the decomposition misses parts of the problem space.

---

### Q8 — Structured error propagation in multi-agent systems
...
**Repo reference:** `modules/module-2/02_structured_error_responses.py`, `modules/module-5/03_context_propagation.py`

**Anti-pattern pair:** C (failure masked as success) and D (all-or-nothing failure) are the two extremes to avoid. The correct approach is structured error with partial results so the coordinator can make an informed decision.

---

### Q9 — Least-privilege tool scoping for subagents
...
**Repo reference:** `modules/module-2/03_tool_scoping.py`

**Principle:** `allowedTools` restricts what each subagent can access. Match tool access to the subagent's actual responsibility, not the maximum possible need.

---

## Scenario 4: Claude Code for Continuous Integration

### Q10 — Non-interactive mode for CI pipelines
...
**Repo reference:** `modules/module-3/06_cli_automation.py`

**CLI flags to know:**
- `-p` / `--print` → non-interactive mode (for CI)
- `--output-format json` → machine-readable output
- `--json-schema` → structured output via schema

---

### Q11 — Message Batches API applicability
...
> Splitting reviews into focused passes directly addresses the root cause: attention dilution when processing many files at once. File-by-file analysis ensures consistent depth, while a separate integration pass catches cross-file issues. Option B shifts burden to developers without improving the system. Option C misunderstands that larger context windows don't solve attention quality issues — attention is still diluted across more content. Option D would actually suppress detection of real bugs by requiring consensus on issues that may only be caught intermittently.

**Repo reference:** `modules/module-4/06_review_architecture.py`

**Multi-pass pattern:** Pass 1 = local issues per file (parallel). Pass 2 = integration issues across files (sequential, needs all local results). Never single-pass a large change.

---

## Answer Key

| Q | Answer | Domain | Primary concept |
|---|--------|--------|----------------|
| 1 | A | Domain 1 | Programmatic enforcement > prompt instructions |
| 2 | B | Domain 2 | Tool descriptions = primary selection mechanism |
| 3 | A | Domain 5 | Explicit escalation criteria with few-shot |
| 4 | A | Domain 3 | `.claude/commands/` for project-scoped commands |
| 5 | A | Domain 3 | Plan mode for multi-file architectural changes |
| 6 | A | Domain 3 | `.claude/rules/` glob patterns for path-scoped rules |
| 7 | B | Domain 1 | Coordinator decomposition scope = coverage of results |
| 8 | A | Domain 2 | Structured error context enables coordinator recovery |
| 9 | A | Domain 2 | Least-privilege tool scoping for subagents |
| 10 | A | Domain 3 | `-p` flag for non-interactive CI mode |
| 11 | A | Domain 4 | Batch API = latency-tolerant only |
| 12 | A | Domain 4 | Multi-pass review for attention quality |

---

## Common Distractor Patterns

The exam consistently uses these wrong-answer patterns. Learn to recognize them:

| Distractor type | Example | Why it's wrong |
|----------------|---------|---------------|
| **Prompt instead of programmatic** | "Add to system prompt" for required tool sequence | Probabilistic, not deterministic |
| **Over-engineering** | Routing classifier / separate ML model | Proportionate fix not tried first |
| **Self-reported confidence** | Agent rates its own confidence 1-10 | LLMs are poorly calibrated on confidence |
| **Sentiment proxy** | Escalate when frustration detected | Sentiment ≠ case complexity |
| **Bigger model = better attention** | Larger context window fixes dilution | Attention quality ≠ context size |
| **Consensus suppresses bugs** | Flag only issues appearing in 2/3 runs | Intermittent detection is real, not noise |
| **Generic status masks failure** | Return "unavailable" after retries | Hides context coordinator needs for recovery |
| **All-or-nothing failure** | Terminate workflow on subagent timeout | Recovery possible with partial results |
